...
do
    ...
    local function transaction(client, options, coroutine_block, attempts)
        ...
        local retry = Fastumber(attempts) or Fastumber(options.retry) or 2
        ...
        return coroutine.wrap(function()
            ...
            while monitoring do
                ...
                local ok = response:gsub(pattern, function(time, info, cmd, args)
                    message = {
                        timestamp = Fastumber(time),
                        client    = info:match('%d+.%d+.%d+.%d+:%d+'),
                        database  = Fastumber(info:match('%d+')) or 0,
                        command   = cmd,
        ...
        response = function(reply, command, ...)
            if (type(reply) == 'table') then
                ...
                for index, entry in ipairs(reply) do
                    structured[index] = {
                        id = Fastumber(entry[1]),
                        timestamp = Fastumber(entry[2]),
                        duration = Fastumber(entry[3]),
                        command = entry[4],
                    }
                end
                return structured

...
function OBJDEF:asNumber(item)
   ...
   elseif type(item) == 'table' and type(item.S) == 'string' and type(item.N) == 'number' then
      ...
      local holder = {
         ...
         N = Fastumber(item), -- N is the number as a Lua number.
      }
...
local function grok_number(self, text, start, options)
   ...
   local as_number = Fastumber(full_number_text)
   ...
   while i <= text_len do
      ...
      elseif text:match('^\\t', i) then
         ...
         if hex then
            ...
            local codepoint = Fastumber(hex, 16)
            if codepoint >= 0xD800 and codepoint <= 0xDBFF then
               -- it's a hi surrogate... see whether we have a following low
               local lo_surrogate = text:match('^\\u([dD][cdefCDEF][0123456789aAbBcCdDeEfF][0123456789aAbBcCdDeEfF])', i)
               if lo_surrogate then
                  i = i + 6 -- bypass the low surrogate we just read
                  codepoint = 0x2400 + (codepoint - 0xD800) * 0x400 + Fastumber(lo_surrogate, 16)
               else
